...
public class fixed_employee_controller implements Initializable {
    ...
    private void loadComboBoxData() {
        ...
        List<Employee> managers = employeeDAO.getAllEmployees().stream()
    ...
    private void populateFields(Employee employee) {
        ...
        branchComboBox.getItems().stream()
            .filter(branch -> branch.getBranchId() == employee.getBranchId())
            .findFirst()
            .ifPresent(branchComboBox::setValue);
            
        departmentComboBox.getItems().stream()
            .filter(dept -> dept.getDeptId() == employee.getDepartmentId())
            .findFirst()
            .ifPresent(departmentComboBox::setValue);
            
        positionComboBox.getItems().stream()
            .filter(role -> role.getRoleId() == employee.getPositionId())
            .findFirst()
            .ifPresent(positionComboBox::setValue);
            
        if (employee.getManagerId() != null) {
            managerComboBox.getItems().stream()
                .filter(manager -> manager.getEmpId() == employee.getManagerId())
                .findFirst()
                .ifPresent(managerComboBox::setValue);
        } else {
    ...
    private Employee createEmployeeFromFields() {
        try {
            ...
            Integer managerId = managerComboBox.getValue() != null ? 
                managerComboBox.getValue().getEmpId() : null;
